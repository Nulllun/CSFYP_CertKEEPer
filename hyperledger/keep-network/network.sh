#!/bin/bash
# This script is modified from Hyperledger Fabric tutorial 

# This script will bring up a Hyperledger Fabric network for CertKEEPer
# The keep network consists of two organizations with one peer for each
# and a single node Raft ordering service. Users can also use this
...
function printHelp() {
  ...
  echo "      - 'deployCC' - deploy the certkeeper chaincode on the channel"
...
function checkPrereqs() {
  ...
  # docker images
  # the binaries version for CertKEEPer are:
  # peer: 2.0.0
  # ca: 1.4.6
...
# CertKEEPer should use CAs for deployment, otherwise, new user cannot be registered
function createOrgs() {
...
function deployCC() {
  ...
  scripts/deployCC.sh $CHANNEL_NAME $CC_RUNTIME_LANGUAGE $CC_NAME $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf organizations/fabric-ca/keep/msp organizations/fabric-ca/keep/tls-cert.pem organizations/fabric-ca/keep/ca-cert.pem organizations/fabric-ca/keep/IssuerPublicKey organizations/fabric-ca/keep/IssuerRevocationPublicKey organizations/fabric-ca/keep/fabric-ca-server.db
    ...
    rm -rf channel-artifacts log.txt certkeeper.tar.gz certkeeper
...
# chaincode name defaults to "certkeeper"
CC_NAME="certkeeper"
